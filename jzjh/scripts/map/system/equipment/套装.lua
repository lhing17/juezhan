...
--- @param h hero
--- @param suit {items:table, hint:string, trick_damage:number, internal:number, real_damage:number, critical_rate:number, }
local function suits_add_attr(h, suit, mode)
    --- @type unit
    ...
    if suit.trick_damage then
        hu:set_str(fun(hu:get_str(), suit.trick_damage))
    end
    if suit.internal then
        hu:set_agi(fun(hu:get_agi()), suit.internal)
    end
    if suit.real_damage then
        hu:set_int(fun(hu:get_int()), suit.real_damage)
    end
end
