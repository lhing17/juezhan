...
function On_Timer()
	...
	for _ in _loop_() do
		...
		SetUnitX(u, check_x(x + 150.0 * Cos(a)))
		SetUnitY(u, check_y(y + 150.0 * Sin(a)))
		...
		if GetUnitAbilityLevel(udg_miejuedanwei, 1093679161) >= 1 then
			...
			loc = GetUnitLoc(u)
			SetUnitX(u, check_x(GetUnitX(udg_miejuedanwei) + 150.0 * Cos(a)))
			SetUnitY(u, check_y(GetUnitY(udg_miejuedanwei) + 150.0 * Sin(a)))
			GroupEnumUnitsInRangeOfLoc(udg_shanghaidanweizu, loc, 100, Condition(MieJue_Condition))
			ForGroupBJ(udg_shanghaidanweizu, MieJue_Action)
			GroupClear(udg_shanghaidanweizu)
			RemoveLocation(loc)
